...
```json
{
  ...
  "storageConfigInfo": {
    "storageType": "S3",
    "allowedLocations": ["s3://sovereign-eu-central/warehouse/regulated"],
...
_Apache Polaris: The Definitive Guide_ (O'Reilly) covers the catalog deployment, identity integration, credential vending, and storage configuration that this article's sovereignty controls depend on, and _Architecting an Apache Iceberg Lakehouse_ (Manning) covers the storage and maintenance side of running one yourself.
