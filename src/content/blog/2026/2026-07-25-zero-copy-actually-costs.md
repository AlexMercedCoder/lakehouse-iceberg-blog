...
_Architecting an Apache Iceberg Lakehouse_ (Manning) covers the storage, catalog, and access patterns that determine when live access beats a copy, and _Apache Polaris: The Definitive Guide_ (O'Reilly) goes deep on the catalog layer where credential vending and cross-catalog federation are decided.
