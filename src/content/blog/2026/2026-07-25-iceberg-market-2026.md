...
_Apache Iceberg: The Definitive Guide_ (O'Reilly) covers the format, the metadata layers, and the maintenance behavior described here from the ground up, and _Apache Polaris: The Definitive Guide_ does the same for the catalog layer where most of the current competition is happening.
