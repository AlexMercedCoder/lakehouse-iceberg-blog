...
tags:
  ...
  - broadcast join
  - hash join database
slug: 2026-04-29-query-engine-09
draft: false
---
...
This is Part 9 of a 10-part series on query engine design. [Part 8](/posts/2026-04-29-query-engine-08/) covered partitioning. This article covers the most expensive operation in distributed query processing: joining two tables whose data lives on different nodes.
...
![Shuffle join redistributing both tables by join key hash so matching keys land on the same node](/assets/images/2026/query-engine-optimization/shuffle-join.png)
...
![Broadcast join copying the small dimension table to every compute node while the large fact table stays in place](/assets/images/2026/query-engine-optimization/broadcast-join.png)
...
![Hash join build and probe phases showing hash table construction from the smaller table and probing from the larger table](/assets/images/2026/query-engine-optimization/hash-join-mechanics.png)
